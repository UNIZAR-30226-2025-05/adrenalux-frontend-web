...
const AchievementList = () => {
  ...
  const allUnlocked =
    achievements.length > 0 && achievements.every((a) => a.unlocked);
  ...
  if (error) {
    const getErrorIcon = () => {
      switch (error.type) {
        case "auth":
          return <FaLock className="text-red-400 text-5xl mb-4" />;
        case "network":
          return <FaWifi className="text-yellow-400 text-5xl mb-4" />;
        case "api":
          return <FaServer className="text-orange-400 text-5xl mb-4" />;
        default:
          return (
            <FaExclamationTriangle className="text-purple-400 text-5xl mb-4" />
          );
      }
    };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        >
          ...
          <h3 className="text-xl font-bold text-white mb-2 text-center">
            {t("achievements.noAchievements.title")}
          </h3>
          <p className="text-gray-300 text-center mb-6">
            {t("achievements.noAchievements.message")}
          </p>
          <button
            onClick={() => navigate("/")}
          ...
          >
            <FaHome /> {t("achievements.noAchievements.goHome")}
          </button>
        </motion.div>
      </div>
    );
              ...
              <div className="flex items-center">
                ...
                <div>
                  <h2 className="text-lg font-bold">
                    {achievement.logro.description}
                  </h2>
                </div>
              </div>
              <div className="text-right">
                <p className="text-green-300 font-semibold">
                  +{achievement.logro.reward_type}
                </p>
                <p className="text-yellow-300 font-semibold">
                  +{achievement.logro.reward_amount} 🪙
                </p>
              </div>
            </motion.div>
          ))}
