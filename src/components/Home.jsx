...
const Home = () => {
  ...
  useEffect(() => {
    const initialize = async () => {
      ...
      intervalId = setInterval(() => {
        ...
        const pct = Math.min((elapsed / MIN_LOADING_TIME) * 100, 100);
        setProgress(pct);
      }, 50);

      // Inicializar socket y perfil
      if (!socketService.socket) {
        const token = getToken();
        const profile = await getProfile();
        await socketService.initialize(token, profile.data.username, navigate);
      }
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 flex flex-col items-center justify-center bg-gradient-to-b from-black via-gray-900 to-black">
        <div
          className="absolute inset-0 opacity-20 bg-center bg-cover"
          style={{ backgroundImage: `url(${background})` }}
        />
        <div className="relative z-10 flex flex-col items-center">
          <h1 className="text-4xl md:text-6xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-blue-500 mb-6 animate-pulse">
            AdrenaLux
          </h1>
          <img
            src={cargaGif}
            alt="Cargando..."
            className="w-36 h-36 md:w-48 md:h-48 mb-8 animate-bounce"
          />
          <div className="w-80 h-4 md:h-6 bg-gray-700 rounded-full overflow-hidden shadow-lg">
            <div
              className="h-full bg-gradient-to-r from-cyan-400 to-blue-600 flicker-animation"
